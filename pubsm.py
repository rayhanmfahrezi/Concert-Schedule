...
def add():
    while True:
        ...
        if (title == "" or is_valid_date(date) == False or is_valid_time(time) == False or performers == ""):
            ...
            client.publish("korean_ballad", to_publish_json)
            client.publish("smtown", to_publish_json2)
...
while True:
    ...
    if choice == "1":
        while True:
            add()
            choice = input("\nPress 'N' to add concert again, Press 'Q' to return to previous menu: ")
            if choice.upper() == 'Q':
                break
